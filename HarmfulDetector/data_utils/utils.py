...
class JSONDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video_path = Path(r'C:\Users\VIET HOANG - VTS\Desktop\VisionReader\HarmfulDetector\data') / entry.get("video_path")
        spec_path = Path(r'C:\Users\VIET HOANG - VTS\Desktop\VisionReader\HarmfulDetector\data') / entry.get("spec_path")
        ...
        with torch.no_grad():
            ...
            text_embed = text_embedding.squeeze(0)  # Shape: (hidden_size,)
    ...
    def _extract_frames(self, file_path):
        ...
        for idx in indices:
            ...
            ret, frame = cap.read()
            if ret:
                frames.append(format_frames(frame, self.output_size))
            else:
                # Add black frame if reading fails
                frames.append(np.zeros((*self.output_size, 3), dtype=np.float32))
                print("hi")
        cap.release()
